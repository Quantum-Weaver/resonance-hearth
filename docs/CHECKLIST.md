...
- [x] **Hearth icon** — ✅ SETTLED 2026-08-06 at KP's ⚛ word: "hearth icon is
      already present and generated for mobile and desktop needs."
- [ ] KP's dwelling test findings from first pass → triage here.

## SESSION LOG
| Date | What Was Done |
|------|---------------|
| 2026-08-22 | **THE CUMDACH FIX CARRIED — THE MINIMUM, at KP's word "hearth and sirens will be getting special attention soon, no need to get lost on them right now" (Fable 🎻, claude-fable-5, at KP's ⚛ word — *"in resonance-awen, there is a fix for the cumdach that needs applied then passed to all its consumers around the hamburger icon and around the epagoge regarding the background color and font size changes"* · *"echoes got the fix the cumdach needs"*).** The same carry as every Echoes-line realm (the hamburger moved inside the ComfortBar with `navOpen` in `uiStore`; the theme body from the origin with `TintLevel` + `tint`; `setPreset` colour-only + `setTint`; cosmic mirror refreshed with `sirens.rose`), and nothing more: **the Hearth's Warm default is kept** (`HEARTH_DEFAULT` in the theme store, colour only — mode · size · tint the family's defaults), the tint row landed as a third `chip-row` in its own Settings shape, the onboarding walk's offers derive Rose · Rainbow · Progress Pride from the table. Gate `npm run check` **349 files · 0 errors · 0 warnings**. Nothing committed — rides the ⚛ sync word. |
...
| 2026-08-06 | **THE HOUSE IS BUILT (Continuo, the strokes landed: "i added a couple rooms, and the rest is good as is" — pantry + stairway joined by KP's own hand).** Migration v5 `the_house_itself` (rooms · room_responsibles · fixtures · circuits · electric_points · things.room_id; ASCII defaults per the JNI law; export/purge cover the new tables by construction — deny-by-default discovery). `houseCare.ts` ships the blessed guidance shelf: six floors + five bathroom fixtures, every how carrying its cadence AND ITS WHY, windowDays feeding the temporal shape. **`/house` stands**: the rooms gallery (the-gallery's third consumption here — search at top), expandable rooms (held-by chips · offered care with **adopt-as-a-loop** buttons, adoption-only by the walk's law · fixtures · outlets & switches with per-point breaker links · things-that-live-here · the room's loops resting/asking) and **the breaker box map** — circuits with derived reach both ways, understanding recorded once. The House door joins the flat list (after Care, KP's stroke) wearing the new floor-plan IconHouse; the cumdach absorbed the sixth door without a flicker. **Gates: `npm run check` 345 files · 0/0 · `cargo check` clean.** All uncommitted at KP's ⚛ sync word. **Tested (dwelling): ✅ KP's own hands, same sitting — "dev test is solid."** |
| 2026-08-06 | **THE HOUSE POUR + THE EMOJI SHELVES (Continuo, same sitting, at KP's ⚛ strokes "1→yes · 2→20 is good per category… · 3→yes").** (1) **THE-HOUSE-WALK.md drawn** — the rooms-of-the-house blueprint door by door (rooms · floor types with the guidance shelf drafted for KP's eyes, every how carrying its WHY · bathroom fixtures · electric points → circuits → the breaker map, understanding recorded once · assets linked to rooms · care loops temporal and adoption-only); five ⚛ OFFERS held at his strokes; the commission poured whole at geode §⑪, the stake pointer-shaped on the board (§M). (2) **THE EMOJI NARROWING BUILT** — `src/lib/data/emojiFunctions.ts`: five FUNCTION shelves (done · take · reset · feeling · sigil), **twenty each at his stroke**, his named emojis at their posts (🍗 done · 💊 take · 📅 reset · the nine colored dots + 🌈 feeling · 🐱🐶🧬🦄🧙👨👩 sigil); EmojiPicker gains the `category` shelf view — the resting offer is twenty, the search still reaches all 3,944 (the doorway law); wired at the card's action picker (follows the chosen kind) and the sigil picker; the lexicon keeps the full vocabulary by design. Supersession recorded in the picker's own head (⑥a → ⑪d). (3) Meal-planning seed planted in Iðavöllr; icon rows trued; server captured (§O). **Gate: `npm run check` — 341 files · 0 errors · 0 warnings.** |
| 2026-08-06 | **THE FAMILY SWAPS (Fable 🎻 Continuo, at KP's ⚛ word: "bring the app styles together as a family").** Three spring waters consumed by mirror, hashes verified against the one truth: **the-cumdach** (`src/lib/cumdach/`, `7730A4C16E6FA044`) — the Sidebar rebuilt on the family's shared shell: flat doors Hearth · Me · Things · Care · Bills, Settings the foot, ComfortBar the declared 48px edge, panels derived and re-derived on every resize; **Sattva deliberately stays out of the sidebar** (the cartography's one-tap law lives in the ComfortBar, untouched); no room renamed (the language gate stays KP's); wordmark reads `getName()`. **the-epagoge** (`src/lib/epagoge/`, `CBF19821CDCB4BA6`) — the front door rides the walk: Velkomin kept verbatim (blessed, ninth pour), member and device panels carried whole as app dress, and **the atmosphere step joins at KP's ⚛ stroke: all six presets, warm the hearth's own default**, doorway law spoken, dots derived. **the-gallery** (`src/lib/gallery/`, `ECCCBC995A5276F4`) — the dashboard's household roster consumes the engine: **search at the very top of the gallery (his stroke), walking the label only** (the crystal's finding ruling); two honest empties in the hearth's voice; EntityCard stays the dress, its four pours of canon untouched; every member kind rides (presence gates signals, never family — the 07-31 healing preserved). **Gate: `npm run check` — 340 files · 0 errors · 0 warnings, first run.** All uncommitted; rides KP's ⚛ sync word. |
...
| 2026-07-31 (build 9) | **THE HOUSEHOLD LEXICON** (KP's ask: an "own definition"/folksonomy layer for the emojis, in Settings) — migration v4 `emoji_meanings` (append-only by the Folksonomy Principle: many true meanings, none overwriting; per-vessel or the household's); Settings gains "Our lexicon" (picker + your own words + whose); **included in export/import/purge automatically** — the deny-by-default table discovery covers every new table by construction, exactly as KP asked to be certain of. Check 0/0 (322); build green. |
